...
void serveClient(int socket)
{
    ...
    while (1)
    {
        ...
        stat("sample.mp4", &file_stats);
        sprintf(sz,"%d",(int)file_stats.st_size);
        send(sockinfo,sz,10,0);

        FILE *filepointer = fopen("sample.mp4", "rb");
        if (!strcmp(accessToken,"XYZ"))
        {
            int s;
...
{   
    ...
    cout<<"This is a VIDEO Server."<<endl;
    ...
    buffer[strlen(buffer)]=':';buffer[strlen(buffer)]='V';buffer[strlen(buffer)]='I';buffer[strlen(buffer)]='D';buffer[strlen(buffer)]='E';buffer[strlen(buffer)]='O';
    ...
    cout<<"Service port number :4200";
    buffer[strlen(buffer)]='4';buffer[strlen(buffer)]='2';buffer[strlen(buffer)]='0';buffer[strlen(buffer)]='0';
    ...
    cout<<"\nAnd the access token to this server is :XYZ\n";
    buffer[strlen(buffer)]='X';buffer[strlen(buffer)]='Y';buffer[strlen(buffer)]='Z';
    ...
    listen(Info, 10);
    cout << "Vedio server is listening on port 4200 ..." << endl;
