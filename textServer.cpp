...
void serveClient(int socket)
{
    ...
    while (1)
    {
        ...
        stat("sample.txt", &file_stats);
        sprintf(sz,"%d",(int)file_stats.st_size);
        send(sockinfo,sz,10,0);

        FILE *filepointer = fopen("sample.txt", "rb");
        if (!strcmp(accessToken,"ABC"))
        {
            int s;
...
{   
    ...
    cout<<"This is a TEXT Server."<<endl;
    ...
    buffer[strlen(buffer)]=':';buffer[strlen(buffer)]='T';buffer[strlen(buffer)]='E';buffer[strlen(buffer)]='X';buffer[strlen(buffer)]='T';
    ...
    cout<<"Service port number :3030";
    buffer[strlen(buffer)]='3';buffer[strlen(buffer)]='0';buffer[strlen(buffer)]='3';buffer[strlen(buffer)]='0';
    ...
    cout<<"\nAnd the access token to this server is :ABC\n";
    buffer[strlen(buffer)]='A';buffer[strlen(buffer)]='B';buffer[strlen(buffer)]='C';
    ...
    {
        cout << "Error occured while binding" << endl;
        exit(0);
    }
    cout << "Bind successfully" << endl;

    listen(Info, 10);
    cout << "Text server is listening on port number 3030 ..." << endl;
