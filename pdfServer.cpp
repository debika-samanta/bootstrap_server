...
void serveClient(int socket)
{
    ...
    while (1)
    {
        ...
        stat("sample.pdf", &file_stats);
        sprintf(sz,"%d",(int)file_stats.st_size);
        send(sockinfo,sz,10,0);

        FILE *filepointer = fopen("sample.pdf", "rb");
        if (!strcmp(accessToken,"DEF"))
        {
            int s;
...
{   
    ...
    cout<<"This is a PDF Server."<<endl;
    ...
    buffer[strlen(buffer)]=':';buffer[strlen(buffer)]='P';buffer[strlen(buffer)]='D';buffer[strlen(buffer)]='F';
    ...
    cout<<"Service port number :4099";
    buffer[strlen(buffer)]='4';buffer[strlen(buffer)]='0';buffer[strlen(buffer)]='9';buffer[strlen(buffer)]='9';
    ...
    cout<<"\nAnd the access token to this server is :DEF\n";
    buffer[strlen(buffer)]='D';buffer[strlen(buffer)]='E';buffer[strlen(buffer)]='F';
    ...
    listen(Info, 10);
    cout << "PDF server is listening on port number 4099.." << endl;
