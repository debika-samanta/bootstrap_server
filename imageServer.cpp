...
void serveClient(int socket)
{
    ...
    while (1)
    {
        ...
        stat("sample.png", &file_stats);
        sprintf(sz,"%d",(int)file_stats.st_size);
        send(sockinfo,sz,10,0);

        FILE *filepointer = fopen("sample.png", "rb+");
        if (!strcmp(accessToken,"IJK"))
        {
            cout<<"idiot";
            int s;
...
{   
    ...
    cout<<"This is a IMAGE Server."<<endl;
    ...
    buffer[strlen(buffer)]=':';buffer[strlen(buffer)]='I';buffer[strlen(buffer)]='M';buffer[strlen(buffer)]='A';buffer[strlen(buffer)]='G';buffer[strlen(buffer)]='E';
    ...
    cout<<"Service port number :2468";
    buffer[strlen(buffer)]='2';buffer[strlen(buffer)]='4';buffer[strlen(buffer)]='6';buffer[strlen(buffer)]='8';
    ...
    cout<<"\nAnd the access token to this server is :IJK\n";
    buffer[strlen(buffer)]='I';buffer[strlen(buffer)]='J';buffer[strlen(buffer)]='K';
    ...
    servaddr.sin_port = htons(2468);
    if (bind(Info, (struct sockaddr *)&servaddr, sizeof(servaddr)) < 0)
    {
        cout << "Error occured in binding" << endl;
        exit(0);
    }
    cout << "Bind successfully" << endl;

    listen(Info, 10);
    cout << "Image server listening on port number 2468 ..." << endl;
